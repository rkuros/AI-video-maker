...
class HighlightGeneratorService {
  ...
  static const HighlightPreferences _defaultPreferences = HighlightPreferences();
  ...
  Future<Timeline> generateBeatBasedHighlight(
    ...
    final selected = _selectBeatSegments(
      scored,
      targetDuration,
      minGapSeconds: max(pattern.minGapSeconds, preferences.minGapSeconds),
      diversityWeight: max(pattern.diversityWeight, preferences.diversityWeight),
    );
  ...
  Future<Timeline> generateTimeBasedHighlight(
    ...
    final selected = _selectTopSegments(diverseSegments, targetDuration)
  ...
  Future<Timeline> generatePatternHighlight(
    ...
    final selectedSegments = _selectTopSegments(
      diverseSegments,
      targetDuration,
    );

    // Adjust pace based on pattern
    final adjustedSegments = _adjustPace(selectedSegments, pattern.paceFactor);

    adjustedSegments.sort((a, b) => a.segment.start.compareTo(b.segment.start));
    return _buildHighlightFromSegments(adjustedSegments);
  }

  Future<List<_SegmentContext>> _extractAdaptiveSegments(Timeline timeline) async {
    ...
    for (final track in timeline.videoTracks) {
      for (final clip in track.clips) {
        ...
        for (var i = 0; i < boundaries.length - 1; i++) {
          ...
          var segStart = start;
          while (segStart < end) {
            final segEnd = _minDuration(
              end,
              segStart + _maxSegmentDuration,
            );
  ...
  Future<List<Duration>> _detectSceneCutsForClip(Clip clip) async {
    ...
    if (stderr.isEmpty) return const [];
    ...
    for (final context in segments) {
      try {
        ...
        final mediaItem = _mediaLibrary.firstWhere(
          ...
          orElse: () => throw Exception('Media item not found: ${context.clip.mediaItemId}'),
        );
  ...
  List<_SegmentContext> _extractBeatSegments(
    ...
    for (final beat in beats) {
      for (final track in timeline.videoTracks) {
        for (final clip in track.clips) {
          if (clip.startTime <= beat.timestamp && beat.timestamp < clip.endTime) {
            ...
            contexts.add(_SegmentContext(segment: segment, clip: clip, track: track));
            break;
          }
        }
      }
    ...
    for (final score in scores) {
      ...
      if (shouldInclude) {
        final sameClipCount = selected.where((s) =>
          s.context.clip.mediaItemId == score.context.clip.mediaItemId
        ).length;

        // Limit segments from same clip based on diversity weight
        final maxSameClip = (10 * (1 - diversityWeight)).round() + 2;
  ...
  Duration _calculateGap(
    Duration start1, Duration end1,
    Duration start2, Duration end2,
  ) {
  ...
  List<_SegmentContext> _selectBeatSegments(
    ...
    final sortedByTime = List<_ScoredSegment>.from(scores)
      ..sort((a, b) => a.context.segment.start.compareTo(b.context.segment.start));
    ...
    for (final score in sortedByTime) {
      ...
      if (ok) {
        final sameClipCount =
            selected.where((s) => s.context.clip.mediaItemId == score.context.clip.mediaItemId).length;
        final maxSameClip = (10 * (1 - diversityWeight)).round() + 2;
  ...
  List<_SegmentContext> _adjustPace(List<_SegmentContext> segments, double paceFactor) {
  ...
  Timeline _buildHighlightFromSegments(
    ...
    if (bgmTrack != null && bgmTrack.filePath.isNotEmpty && currentTime > Duration.zero) {
